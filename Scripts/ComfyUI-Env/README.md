# 🇺🇦 ComfyUI Runtime Environment Manager (Ukrainian)
...
- Скрипт **запускається через `ComfyUI-Env.bat`**, який повинен лежати в тій же папці, де знаходиться скрипт `ComfyUI-Env.ps1`.  
...
# 🇬🇧 ComfyUI Runtime Environment Manager (English)
...
- The script **is run via `ComfyUI-Env.bat`**, which must be in the same folder as the script `ComfyUI-Env.ps1`.  
